...
all_weekly_answers_completed = all_weekly_answers_completed.split('\n');
all_special_answer_completed = all_special_answer_completed.split('\n');
var { whether_complete_subscription } = hamibot.env;
whether_complete_subscription = whether_complete_subscription.split('\n');
var { whether_complete_speech } = hamibot.env;
whether_complete_speech = whether_complete_speech.split('\n');
...
// 保存各账号信息
var { account_information } = hamibot.env;
account_information = account_information.split('\n');
var phone_number_list = [];
var account_password_list = [];
for (var i = 0; i < account_information.length; i++) {
    if (i % 2 == 0) phone_number_list.push(account_information[i]);
    else account_password_list.push(account_information[i]);
}
 ...
 */
function switch_account(phone_number, account_password) {
    id("my_setting").waitFor();
    id("my_setting").findOne().click();
    sleep(random_time(delay_time));
    my_click_clickable('退出登录');
    sleep(random_time(delay_time));
    my_click_clickable('确认');
    sleep(random_time(delay_time));
    id("et_phone_input").waitFor();
    id("et_phone_input").findOne().setText(phone_number);
    id("et_pwd_login").waitFor();
    id("et_pwd_login").findOne().setText(account_password);
    sleep(random_time(delay_time));
    my_click_clickable('登录');
}
...
function map_update() {
    toast("正在下载题库");
    // 使用 Github 文件加速服务：https://ghproxy.com
...
for (var account_i = 0; account_i < phone_number_list.length; account_i++) {
    /*
     *********************切换账号********************
     */
    // 回到首页
    var back_track_wait_time = 2;
    back_track_flag = 1;
    back_track();
    my_click_clickable("我的");
    sleep(random_time(delay_time));
    switch_account(phone_number_list[account_i], account_password_list[account_i]);
    ...
    */
    ...
    if (typeof (finish_dict['每周答题']) != "undefined" && !finish_dict['每周答题'][0] && finish_dict['每周答题'][1] < 4 && all_weekly_answers_completed[account_i] == "no") {
        log("每周答题");
        sleep(random_time(delay_time));
        if (!className("android.view.View").depth(22).text("学习积分").exists()) back_track();
        entry_model('每周答题');
        ...
        // 打开第一个出现未作答的题目
        // 如果之前的答题全部完成则不向下搜索
        if (all_weekly_answers_completed[account_i] == "no") {
    ...
    if (typeof (finish_dict['专项答题']) != "undefined" && finish_dict['专项答题'][1] == 0) {
        ...
        // 如果之前的答题全部完成则不向下搜索
        if (all_special_answer_completed[account_i] == "yes") {
            special_flag = true;
        }
        while (!special_flag) {
            if (text("开始答题").exists()) {
     ...
     **********订阅*********
     */
    while (typeof (finish_dict['订阅']) != "undefined" && !finish_dict['订阅'][0] && whether_complete_subscription[account_i] == "yes") {
        log("订阅");
        sleep(random_time(delay_time));
        if (!className("android.view.View").depth(22).text("学习积分").exists()) back_track();
        entry_model('订阅');
    ...
    if (typeof (finish_dict['发表观点']) != "undefined" && !finish_dict['发表观点'][0] && whether_complete_speech[account_i] == "yes") {
